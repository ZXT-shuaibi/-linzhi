...
| storage | ✅ 已实现（基础版） | 已支持独立预签名上传入口、帖子正文/图片上传场景、头像上传场景、公开 URL 统一生成 |
| cache | ✅ 已实现（基础版） | 已抽离独立缓存支持模块，统一承接本地缓存与 Redis JSON/String 读写，并已接入 feed、discover、trade |
| search | ✅ 已实现（基础版） | 已支持公开帖子关键词搜索、标签过滤、简化版游标分页、联想建议和基于距离的结果过滤 |
| profile | ✅ 已实现（基础版） | 已支持资料查询、资料编辑、头像单独更新、个人主页信息聚合、个人发布列表、关注/粉丝资料列表 |
| llm | ✅ 已实现（基础版） | 已提供帖子描述生成能力，并接入 rag 问答链作为统一回答组织层，同时具备 template/http provider 可切换的接模骨架 |
| rag | ✅ 已实现（基础版） | 已支持基础版流式问答与单篇索引重建入口 |
| trade | ✅ 已实现（基础版） | 已支持活动创建/查询、Redis+Lua 预扣、异步下单、支付、主动取消、主动/被动关单、补偿回写 |
| platform | ✅ 已实现（基础版） | 已补平台运行摘要与手动缓存清理入口，作为轻量治理层统一出口 |
